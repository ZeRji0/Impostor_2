...
// Banco5000 loader
async function cargarBanco5000(){
  try{
    const r = await fetch("banco5000.json", {cache:"no-store"});
    if(!r.ok) throw 0;
    const arr = await r.json();
    if(Array.isArray(arr)){
      // Filtrar a strings simples y quitar vacíos
      BANCO5000 = arr.map(x=>String(x).trim()).filter(Boolean);
    }
  }catch(e){
    BANCO5000 = [];
  }
...
$("#btn-contar-banco")?.addEventListener("click", ()=>{
  const n = BANCO5000?.length || 0;
  $("#label-banco").textContent = n ? `${n} palabras disponibles` : "No cargado o vacío";
});

// Cargar al inicio
document.addEventListener("DOMContentLoaded", ()=>{ cargarBanco5000(); });
// Impostores
...
$("#btn-comenzar").onclick = async ()=>{
  ...
  try{
    if(state.modo === "mis"){
      ...
      shuffle(arr);
      state.palabra = arr[0];
    } else if (state.modo === "banco5000"){
      if(!BANCO5000 || !BANCO5000.length){
        // Si no hay banco cargado, usamos LOCAL_ES como respaldo
        BANCO5000 = LOCAL_ES.slice();
      }
      state.palabra = BANCO5000[Math.floor(Math.random()*BANCO5000.length)];
